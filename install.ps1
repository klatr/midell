...
# ---- Fluent Terminal ----
# Fluent Terminal needs to be installed as normal user, same as Spotify.
# We use explorer.exe as a launcher to drop admin context.
Write-Step "Installing Fluent Terminal (as normal user)..."
try {
    $result = winget install --id felixse.FluentTerminal -e --accept-source-agreements --accept-package-agreements 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Success "Fluent Terminal installed"
    } elseif ($result -match "already installed") {
        Write-Skipped "Fluent Terminal"
    } else {
        # Fallback: try via explorer.exe to drop admin
        Write-Step "Retrying Fluent Terminal via user context..."
        Start-Process "explorer.exe" -ArgumentList "shell:appsFolderelixse.FluentTerminal_8wekyb3d8bbwe!App" -Wait -ErrorAction SilentlyContinue
        $result2 = winget install --id felixse.FluentTerminal -e --accept-source-agreements --accept-package-agreements 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Success "Fluent Terminal installed"
        } else {
            Write-Fail "Failed to install Fluent Terminal - install manually from: https://github.com/felixse/FluentTerminal/releases"
        }
    }
} catch {
...
try {
    ...
    spicetify config current_theme matte --bypass-admin
...
try {
    ...
    spicetify config extensions fullScreenWrapper.js --bypass-admin
...
try {
    Start-Process powershell -ArgumentList "-NoProfile -ExecutionPolicy Bypass -Command `"spicetify backup apply --bypass-admin`"" -Wait
